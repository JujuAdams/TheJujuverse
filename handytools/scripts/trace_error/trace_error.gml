...
if ( TRACE_SHOW_ERROR ) {
    ...
    trace_vf( "ERROR", TRACE_DIV, string_replace_all( _str, NL, TRACE_DIV ) );
    ...
    for( var _i = _start; _i < argument_count; _i++ ) _str += string_ext( argument[_i] );
    if ( _abort ) {
        trace_vf( "ERROR", TRACE_DIV, string_replace_all( _str, NL, TRACE_DIV ), TRACE_DIV, "((", debug_get_callstack_unpacked( "  ||  ", "" ), "))" );
    } else {
