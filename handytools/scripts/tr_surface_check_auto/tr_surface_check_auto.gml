...
if ( _width == undefined ) {
	...
	if ( surface_exists( _index ) ) {
		...
		if ( TRACKER_DEBUG_SURFACE_CHECK ) trace_v( "existing surface ", _index, " newly registered in tracker (surface_check_auto)", TRACE_DEFAULT_DIVIDER, tr_surface_details( _index, DEFAULT ) );
		return _index;
	}
...
if ( _regen ) {
	...
	if ( TRACKER_ON and TRACKER_DEBUG_SURFACE_CHECK ) trace_v( "regen surface=", _index, TRACE_DEFAULT_DIVIDER, tr_surface_details( _index, DEFAULT ) );
